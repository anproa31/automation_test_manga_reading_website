...
test('TC_SignUp_12: Sign up with SQL injection in username', async (t) => {
    const user = new TestUser();
    user.username = "admin'; DROP TABLE users;--" + user.username;
    user.log();
    ...
    await t
        ...
        .ok('Expected rejection for destructive SQL injection in username');

    await t.takeScreenshot({ path: 'screenshots/TC_SignUp_12.png' });
});